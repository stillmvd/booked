...
/// Отказ до первой записи держится сигнатурой ядра: `backup::parse` не принимает соединение
/// с базой (T-07-36). Текущие числа базы читаются отдельным, только читающим запросом —
/// после того, как файл уже прошёл проверку ядра.
#[tauri::command]
...
/// Разбор выполняется заново: между показом сводки и нажатием кнопки файл мог измениться,
/// применять устаревший разбор нельзя.
#[tauri::command]
