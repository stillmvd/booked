...
pub fn db_reveal(app: AppHandle) -> Result<(), String> {
    ...
    tauri_plugin_opener::reveal_item_in_dir(dir.join("booked.db")).map_err(|e| e.to_string())
}
...
pub fn db_start_fresh(app: AppHandle, db: State<Db>) -> Result<(), String> {
    ...
    let result = booked_core::db::start_fresh_at(&dir);
